...
def archive_inbox(session: Session, archive_individual_messages: bool) -> None:
    ...
    while True:
        ...
        print(f'Archived {messages_archived:,}/{total_messages:,} messages from INBOX ({current_page} pages) so far. {(total_pages - current_page):,} pages ({(total_messages - messages_archived):,} messages) remain.')
