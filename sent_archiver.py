...
def archive_sent(session: Session, archive_individual_messages: bool) -> None:
    ...
    while True:
        ...
        print(f'Saved {messages_archived:,}/{total_messages:,} messages from SENT ({current_page} pages) so far. {(total_pages - current_page):,} pages ({(total_messages - messages_archived):,} messages) remain.')
