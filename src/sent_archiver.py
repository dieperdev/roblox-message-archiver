...
def archive_sent(session: Session, archive_individual_messages: bool, cursor: Cursor) -> None:
    page_offset = 0
    ...
    while True:
        ...
        for message in sent_messages:
            ...
            cursor.execute('INSERT INTO sent (message_id, sender_id, sender_verified, sender_username, sender_display_name, recipient_id, recipient_verified, recipient_username, recipient_display_name, read, system_message, is_report_abuse_displayed, created, updated, subject, body) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT (message_id) DO NOTHING;', (message_id, message_data['sender']['id'], message_data['sender']['verified'], message_data['sender']['username'], message_data['sender']['display_name'], message_data['recipient']['id'], message_data['recipient']['verified'], message_data['recipient']['username'], message_data['recipient']['display_name'], message_data['read'], message_data['system_message'], message_data['isReportAbuseDisplayed'], message_data['created'], message_data['updated'], message_data['subject'], message_data['body']))

            messages_archived += 1

        print(f'Saved {messages_archived:,}/{total_messages:,} messages ({current_page} pages) from the SENT category so far. {(total_pages - current_page):,} pages ({(total_messages - messages_archived):,} messages) remain.')
